...
class Table {
  constructor(globalApplicationState) {
    ...
    this.headerData = [
      ...
      },
      {
        ...
        ascending: false,
        description:
          "Average rating on Mountain Project, an online database of climbing routes. Ranges from 0-4 stars",
      },
      {
        key: "totalViews",
        secondKey: "avgRating",
        ...
        ascending: false,
        description:
          "Total number of views on boulder problem's Mountain Project page, an online database of climbing routes.",
      },
    ];
  ...
  drawLegend(data) {
    ...
    const drawViewsAxis = d3
      .select("#viewsAxis")
      .attr("height", axisHeight)
      .attr("width", this.viewWidth)
      .call(
        d3
          .axisBottom(this.viewsScaleX)
          .ticks(8)
          .tickFormat((d) =>
            d < 10001 ? d3.format(".0s")(d) : d3.format(".2s")(d)
          )
      );
  ...
  drawTable(data) {
    ...
    this.currentData.map(
      (d) => (d.gradeNumber = parseInt(d.grade.substring(1)))
    );
    ...
    let ratingSvgSelect = ratingSelection
      .selectAll("svg")
      .data((d) => [d])
      .join("svg")
    ...
    let viewSvgSelect = viewSelection
      .selectAll("svg")
      .data((d) => [d])
      .join("svg")
      .attr("width", this.viewWidth)
  ...
  addViewsRects(svg) {
    ...
    drawRect
      ...
      .attr("height", this.rowHeight)
      .attr("fill", (d) =>
        d.affected ? "rgb(101, 101, 101)" : "rgb(152, 105, 213)"
      )
      .attr("opacity", "1");
  }
  ...
  attachSortHandlers() {
    ...
    headerSelection.on("click", (event, d) => {
      ...
      this.headerData
        .filter((s) => s !== d)
        .forEach((col) => (col.sorted = false));
      this.headerData
        .filter((s) => s === d)
        .forEach((col) => (col.sorted = true));
      ...
      if (d.ascending === false) {
        ...
        this.headerData
          .filter((s) => s === d)
          .forEach((col) => (col.ascending = true));
        ...
        this.headerData
          .filter((s) => s === d)
          .forEach((col) => (col.ascending = false));
        this.drawTable(this.currentData);
      }
    });
